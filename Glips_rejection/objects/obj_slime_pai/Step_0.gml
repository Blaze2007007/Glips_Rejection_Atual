...
if(variable_global_exists("dead") && variable_global_exists("gamepaused"))
{
...
if(instance_exists(obj_bg) && instance_exists(obj_sala_secreta_bg))
{
	// Limites de transição
	var x1 = 2432; // início (sala principal visível)
	var x2 = 2476; // fim (sala secreta totalmente visível)
	
	// Fator de interpolação limitado entre 0 e 1
	var t = clamp((x - x1) / (x2 - x1), 0, 1);
	if(!inmenu)
	{
		// Alphas com base em t
		obj_sala_secreta_bg.image_alpha = 1 - t;
		obj_bg.image_alpha = t;
	}
	else
	{
		if(t <= 0.5)
		...
		{
			obj_sala_secreta_bg.image_alpha = 0.75
		}
	}
}
